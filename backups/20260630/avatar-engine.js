...
const DEFAULT_AVATAR = 'https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&w=800';
...
const server = http.createServer(async (req, res) => {
  ...
  if ((req.url === '/avatar/speak' || req.url === '/speak') && req.method === 'POST') {
    ...
    req.on('end', async () => {
      try {
        const { text, lang, avatar_url } = JSON.parse(body);
        const result = await createTalk(text, lang||'ko-KR', avatar_url);
        res.writeHead(200, {'Content-Type':'application/json'});
  ...
  if ((req.url.startsWith('/avatar/status/') || req.url.startsWith('/status/')) && req.method === 'GET') {
    ...
    try {
      const result = await didRequest('GET', `/talks/${talkId}`, null);
      res.writeHead(200, {'Content-Type':'application/json'});
      res.end(JSON.stringify(result));

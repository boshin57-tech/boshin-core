module tobmate_finance::oracle_feed_aggregator;

use tobmate_finance::oracle::{
    OracleRegistry,
};
use tobmate_finance::oracle_feed::{
    OracleFeedStore,
};
use tobmate_finance::oracle_aggregator::{
    OracleAggregator,
};
...
public fun aggregate_feed_round(
    ...
    assert!(
        !tobmate_finance::oracle::is_paused(registry),
        E_PROTOCOL_PAUSED,
    );

    assert!(
        tobmate_finance::oracle_feed::store_registry_id(store)
            == tobmate_finance::oracle::registry_id(registry),
        E_INVALID_FEED_STORE,
    );

    assert!(
        tobmate_finance::oracle_feed::feed_exists(store, feed_id),
        E_FEED_NOT_FOUND,
    );

    assert!(
        !tobmate_finance::oracle_feed::feed_is_paused(
            store,
            feed_id,
        ),
        E_FEED_PAUSED,
    ...
    ) = tobmate_finance::oracle_feed::round_observation_snapshot(
        store,
        feed_id,
        round,
    );
    ...
    assert!(
        raw_observation_count
            >= tobmate_finance::oracle_aggregator::min_observations(
                aggregator,
            ),
    ...
    let heartbeat_ms =
        tobmate_finance::oracle_feed::feed_heartbeat_ms(
            store,
            feed_id,
        );
    ...
    let raw_median_price =
        tobmate_finance::oracle_aggregator::calculate_median(
            copy active_prices,
        );
    ...
    let (
        ...
        active_confidences,
        raw_median_price,
        tobmate_finance::oracle_aggregator::max_deviation_bps(
            aggregator,
        ),
    );
    ...
    let weighted_median_price =
        tobmate_finance::oracle_aggregator::calculate_weighted_median(
            copy accepted_prices,
    ...
    tobmate_finance::oracle_aggregator::aggregate_round(
        aggregator,
        accepted_prices,
        accepted_weights,
        timestamp_ms,
    ...
    tobmate_finance::oracle_feed::update_canonical_price(
        store,
        feed_id,
        round,
...
fun collect_active_observations(
    ...
    while (index < length) {
        ...
        let publisher_exists =
            tobmate_finance::oracle::publisher_exists(
                registry,
                publisher_id,
            );
        ...
        let publisher_active =
            publisher_exists
                && tobmate_finance::oracle::publisher_is_active(
                    registry,
                    publisher_id,
                );
        ...
        if (publisher_active && fresh) {
            let weight =
                tobmate_finance::oracle::publisher_weight(
                    registry,
                    publisher_id,
                );
...
fun filter_outliers(
    ...
    while (index < length) {
        ...
        let deviation_bps =
            tobmate_finance::oracle_aggregator::calculate_deviation_bps(
                price,
                reference_median,
            );

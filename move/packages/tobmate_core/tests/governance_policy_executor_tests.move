#[test_only]
module tobmate_core::governance_policy_executor_tests;
...
fun test_07_recovery_threshold_execution_succeeds() {
    ...
    let mut recovery_registry =
        tobmate_core::treasury_strategy_recovery_mode::new_for_testing(
            test_scenario::ctx(&mut scenario),
        );

    let recovery_cap =
        tobmate_core::treasury_strategy_recovery_mode::admin_cap_for_testing(
            &recovery_registry,
            test_scenario::ctx(&mut scenario),
        );
    ...
    assert!(
        tobmate_core::treasury_strategy_recovery_mode::recovery_threshold_bps(
            &recovery_registry,
        ) == 7_000,
        700,
    );
    ...
    tobmate_core::treasury_strategy_recovery_mode::destroy_admin_cap_for_testing(
        recovery_cap,
    );

    tobmate_core::treasury_strategy_recovery_mode::destroy_for_testing(
        recovery_registry,
    );

...
const AVATAR_URLS = {
  male1:   'https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&w=800',
  male2:   'https://images.pexels.com/photos/1681010/pexels-photo-1681010.jpeg?auto=compress&cs=tinysrgb&w=800',
  female1: 'https://images.pexels.com/photos/415829/pexels-photo-415829.jpeg?auto=compress&cs=tinysrgb&w=800',
  female2: 'https://images.pexels.com/photos/3769021/pexels-photo-3769021.jpeg?auto=compress&cs=tinysrgb&w=800',
  female3: 'https://images.pexels.com/photos/1820919/pexels-photo-1820919.jpeg?auto=compress&cs=tinysrgb&w=800',
};
const DEFAULT_AVATAR_MALE = AVATAR_URLS.male1;
const DEFAULT_AVATAR_FEMALE = AVATAR_URLS.female1;
...
async function createTalk(text, lang, avatarUrl, gender) {
  const data = {
    ...
    script: {
      ...
      provider: {
        type: 'microsoft',
        voice_id: lang === 'ko-KR'
          ? (gender==='female' ? 'ko-KR-SunHiNeural' : 'ko-KR-InJoonNeural')
...
const server = http.createServer(async (req, res) => {
  ...
  if ((req.url === '/avatar/speak' || req.url === '/speak') && req.method === 'POST') {
    ...
    req.on('end', async () => {
      try {
        const { text, lang, avatar_url, gender, avatar_key } = JSON.parse(body);
        const autoAvatar = AVATAR_URLS[avatar_key] || (gender==='female' ? DEFAULT_AVATAR_FEMALE : DEFAULT_AVATAR_MALE);
        const result = await createTalk(text, lang||'ko-KR', avatar_url||autoAvatar, gender||'male');

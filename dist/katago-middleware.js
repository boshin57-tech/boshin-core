...
function connectBotToRoom(roomId) {
    ...
    botSocket.on('connect', function() {
        console.log('[BOT] 소켓 연결됨 id:', botSocket.id);
        setTimeout(function() {
            console.log('[BOT] 788 전송:', roomId);
            botSocket.emit('cs_message', 788, {token:BOT_TOKEN, room_id:roomId});
        }, 500);
    });

    botSocket.on('connect_error', function(err) {
        console.log('[BOT] connect_error:', err && err.message ? err.message : err);
    });

    botSocket.on('connect_timeout', function(timeout) {
        console.log('[BOT] connect_timeout:', timeout);
    });

    botSocket.on('error', function(err) {
        console.log('[BOT] socket error:', err && err.message ? err.message : err);
    });

    botSocket.on('reconnect_attempt', function(attempt) {
        console.log('[BOT] reconnect_attempt:', attempt);
    });

    botSocket.on('reconnect_failed', function() {
        console.log('[BOT] reconnect_failed');
    });
